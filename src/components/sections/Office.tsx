    ...
    <section id="office" className="w-full bg-[var(--color-surface)] py-[var(--spacing-space-xl)] lg:py-28 scroll-mt-20">
      <div className="mx-auto max-w-[1240px] px-[var(--spacing-gutter)]">
        <div className="max-w-3xl">
          ...
          <p className="mt-4 text-[var(--color-on-surface-variant)]" style={{ fontSize: '18px', lineHeight: '28px' }}>
        ...
        <div className="mt-10 sm:mt-14 grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-8">
          {/* In-Person Card */}
          <div className="rounded-3xl bg-[var(--color-surface-container-low)] p-6 sm:p-8 lg:p-10 flex flex-col justify-between shadow-sm">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full bg-[var(--color-surface)] px-3.5 py-1 text-[var(--color-primary)]">
              ...
              <h3 className="mt-5 sm:mt-6 text-[var(--color-primary)]" style={{ fontFamily: 'var(--font-newsreader)', fontSize: 'clamp(24px, 3.5vw, 28px)', lineHeight: '1.25', fontWeight: 500, letterSpacing: '-0.01em' }}>
                In-Person Santa Monica Sanctuary
              </h3>
              <p className="mt-4 text-[var(--color-on-surface-variant)] leading-relaxed text-sm sm:text-base" style={{ lineHeight: '1.6' }}>
              ...
              <ul className="mt-6 space-y-3">
                {inPersonFeatures.map(({ label, value }) => (
                  <li key={label} className="flex items-start gap-2.5 text-[var(--color-on-surface-variant)] text-sm" style={{ lineHeight: '22px' }}>
                    <CheckCircle2 size={18} className="text-[var(--color-secondary)] shrink-0 mt-0.5" strokeWidth={1.8} />
                    <span><strong className="text-[var(--color-on-surface)]">{label}:</strong> {value}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="mt-8 pt-6 border-t border-[var(--color-surface-variant)] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="mt-8 rounded-3xl bg-[var(--color-surface-container)] overflow-hidden shadow-sm border border-[var(--color-outline-variant)]/30">
          <div className="grid grid-cols-1 lg:grid-cols-12 items-stretch">
            {/* Real Interactive Google Map */}
            <div className="lg:col-span-8 min-h-[280px] sm:min-h-[340px] lg:min-h-[420px] w-full relative overflow-hidden bg-[var(--color-surface-container-high)]">
              <iframe
                title="Dr. Maya Reynolds Office Location - Google Maps"
                src={mapEmbedUrl}
                className="w-full h-full min-h-[280px] sm:min-h-[340px] lg:min-h-[420px] border-0 block"
                loading="lazy"
                allowFullScreen
            ...
            {/* Directions Sidebar */}
            <div className="lg:col-span-4 p-6 sm:p-8 lg:p-10 flex flex-col justify-between bg-[var(--color-surface)]">
              <div>
                ...
                <h4 className="text-[var(--color-primary)]" style={{ fontFamily: 'var(--font-newsreader)', fontSize: '24px', lineHeight: '32px', fontWeight: 500 }}>
                  Easy Coastal Access
                </h4>
                <p className="mt-2 text-[var(--color-on-surface)] font-medium text-sm">
                  {CONTACT_INFO.address}
                </p>
                <p className="mt-3 text-[var(--color-on-surface-variant)] leading-relaxed text-sm">
                ...
                <div className="mt-5 space-y-2.5">
                  <p className="flex items-center gap-2 text-[var(--color-on-surface)] text-xs font-medium">
                    <Footprints size={16} className="text-[var(--color-secondary)] shrink-0" strokeWidth={1.8} />
                    4 min walk to Downtown Santa Monica Expo Station
                  </p>
                  <p className="flex items-center gap-2 text-[var(--color-on-surface)] text-xs font-medium">
                    <ParkingSquare size={16} className="text-[var(--color-secondary)] shrink-0" strokeWidth={1.8} />
                    Structure 4 and on-site subterranean parking
                  </p>
                </div>
              </div>

              <div className="mt-8 pt-6 border-t border-[var(--color-outline-variant)]/30 flex flex-col gap-2">
                <a
