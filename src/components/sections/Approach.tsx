...
export default function Approach() {
  return (
    <section id="approach" className="w-full bg-[var(--color-surface-container)] py-12 sm:py-20 lg:py-28 scroll-mt-20">
      <div className="mx-auto max-w-[1240px] px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-8 sm:mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {APPROACH_STEPS.map(({ step, title, description, footer }) => (
            <div
              key={step}
              className="relative rounded-2xl bg-[var(--color-surface)] p-5 sm:p-7 shadow-sm flex flex-col justify-between hover:shadow-md hover:-translate-y-0.5 transition-all duration-300"
            >
              <div>
                <div className="flex items-center">
                  <span
                    className="flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full bg-[var(--color-secondary-container)] text-[var(--color-secondary)] font-semibold"
